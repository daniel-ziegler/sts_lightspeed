...
from lightspeed.network import NN, ModelHP, move_to_device, process_batch, choice_space, collate_fn, load_network_backward_compatible, SeparateValuePolicy, EventFixedInfo, CHOICE_PATHS_OFFSET
from lightspeed.playouts import run_game, NNService, Choice, Decision, ActionType, ChoiceStats, path_to_action_and_desc, construct_choice, take_free_rewards, choose_overworld_action, flatten_dict
from lightspeed.algorithms import (
...
def train_step(nets, optimizer, experiences: List[Experience], advantages: List[float], returns: List[float], config: TrainConfig, algo=None, iteration: int = -1):
    ...
    if not experiences:
        return {}
    if algo is None:
        from lightspeed.algorithms import PPOAlgorithm
        algo = PPOAlgorithm()

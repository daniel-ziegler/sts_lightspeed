...
from lightspeed.network import NN, ActionType, FixedAction, EventFixedInfo, ModelHP, collate_fn, process_batch, output_to_cpu, choice_space, move_to_device, load_network_backward_compatible
from lightspeed.inputs import Path
...
class Choice:
    ...
    screen_state: int  # snapshot int -- NOT gc.screen_state, which pybind returns as a live
    select_screen_type: int  # alias to the C++ field (reads reflect later mutation). See construct_choice.
    ...
    def as_dict(self):
        ...
        return dict(
            ...
            select_screen_type=self.select_screen_type,
        )
...
def choose_overworld_action(service, choice: Choice, gc: sts.GameContext, rng,
    ...
    from lightspeed.network import CHOICE_PATHS_OFFSET
...
def construct_choice(gc: sts.GameContext, obs: sts.NNRepresentation, actions: list[sts.GameAction]) -> Optional[Choice]:
    ...
    return Choice(obs, cards_offered=cards_offered, card_actions=card_actions,
                  ...
                  screen_state=int(gc.screen_state), select_screen_type=int(gc.screen_state_info.select_screen_type))

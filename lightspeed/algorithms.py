"""RL algorithm strategies and the loss helpers they share.
...
class CollectionJob(NamedTuple):
    ...
    action-sampling RNG (decoupled so replicates can diverge on the same map)."""
    game_seed: int
    sample_seed: int
...
def clipped_surrogate(ratio, advantages, clip_ratio):
    """Clipped surrogate policy loss: -mean(min(r*A, clip(r)*A))."""
...
class PPOAlgorithm(Algorithm):
    ...
    def collection_plan(self, config, iteration):
        # One game per seed; sample_seed == game_seed reproduces the original
        # start_seed = iteration*1000 collection exactly (RNG was tied to the game seed).
        base = iteration * 1000
        return [CollectionJob(game_seed=base + i, sample_seed=base + i)
    ...
    def compute_advantages(self, trajectories, config, adv_norm, debug_traj=False):
        ...
        from lightspeed.rl_train import compute_advantages as _gae

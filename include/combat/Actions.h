...
namespace sts {
    ...
    struct _MakeTempCardInDrawPile { const CardInstance *c; int amount; bool shuffleInto; void operator()(BattleContext& bc) const; };
    struct _MakeTempCardInDiscard { const CardInstance *c; int amount = 1; void operator()(BattleContext& bc) const; };
    ...
    struct Actions {
        ...
        static Action MakeTempCardInHand(CardId card, bool upgraded=false, int amount=1) {
            return Action(_MakeTempCardInHand {card, upgraded, amount});
        }
        static Action MakeTempCardInHand(CardInstance c, int amount=1) {
            return Action(_MakeTempCardInHandInstance {c, amount});
        }
        ...
        static Action DamageAllEnemy(int damage);
